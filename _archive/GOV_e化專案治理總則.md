...
---
title: "[文件標題]"
type: [Discovery | Analysis | Spec | Archive]
status: [Draft | Review | Final]
tags: [關鍵字1, 關鍵字2, 客戶或專案名]
version: 1.0
owner: [負責人]
...
|:---|:---|
| **title** | 文件顯示標題 |
| **type** | Discovery=診斷期 / Analysis=分析期 / Spec=規格或手冊 / Archive=歸檔 |
| **status** | 草稿 / 審閱中 / 定稿 |
| **tags** | 便於搜尋與分類，可含客戶名、主題 |
| **version** | 語意化版本或序號 |
| **owner** | 負責維護者 |
| **last_updated** | 最後更新日期 |
...
- **新文件優先參考：** 從 `00_Templates` 選擇對應範本再撰寫。

### 2.3 流程與內部標準（需求→交付包）

從**公司背景／需求**到**釋出協助開發包給工程師以 AI 協作開發**之完整生命週期與手off，見 **internal_docs/需求到交付包_生命週期與流程總覽.md**。公司標準、協作規範、程式編碼、交付包結構與產出步驟以 **internal_docs/** 為**單一真相來源**；_Inbox 內待歸檔檔案，歸檔後以 internal_docs 為準，避免雙版本並存。
...
- **治理與範本：** 由專案負責人維護；本總則與 `00_Templates` 為單一真相來源。
...
| [internal_docs/需求到交付包_生命週期與流程總覽.md](./internal_docs/需求到交付包_生命週期與流程總覽.md) | **需求→RFP→PRD→程式編碼→雛型→交付包→釋出** 完整流程與手off；各階段產出與關鍵文件索引。 |
| [internal_docs/](./internal_docs/) | **公司標準、協作、程式編碼、交付包、AI 產出** 之單一真相來源；含 README、index、standards。 |
| [GOV_Inbox_歸檔審視與建議.md](./GOV_Inbox_歸檔審視與建議.md) | _Inbox 依範本方式之歸檔審視與建議、目錄結構。 |
| [OPS_Inbox_AutoSorter.md](./OPS_Inbox_AutoSorter.md) | Inbox 自動歸檔執行手冊。 |
| [OPS_Publish_To_Web.md](./OPS_Publish_To_Web.md) | 文件發布至網站執行手冊。 |
| [00_Templates/](./00_Templates/) | 訪談、RFP、PRD、交付包、驗收、需求變更等範本。 |
| [01_Discovery/](./01_Discovery/)、[02_Analysis/](./02_Analysis/) | 診斷期／分析期產出目錄；各含 README 說明用途與生命週期銜接。 |
...
| **範本補完** | `00_Templates/Tpl_Solution_Spec.md` 內 Mermaid 區塊未閉合，建議補齊並新增「分析報告」範本 (Tpl_Analysis_Report.md)。 | 中 |
| **單一入口** | 在 README 或 `docs/index.md` 明確寫出「治理總則請見 GOV_e化專案治理總則.md」，方便新成員與 AI 快速定位。 | 中 |
